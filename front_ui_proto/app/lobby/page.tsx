      ...
      <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-black text-white">ロビー</h1>
              <p className="text-sm text-white/80">プレイヤーを待っています...</p>
            </div>
          </div>
          <Button
            variant="ghost"
            size="icon"
            onClick={handleLeave}
            className="text-white/70 hover:text-white hover:bg-card/50"
          >
        ...
        <div className="bg-card/50 backdrop-blur-sm border-2 border-border rounded-xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-bold text-white flex items-center gap-2">
              ...
              参加プレイヤー
              <span className="text-sm font-normal text-white/70">({players.length}/12)</span>
            </h2>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {Array.from({ length: Math.max(0, 12 - players.length) }).map((_, i) => (
              ...
              >
                <span className="text-xs text-white/60">空き</span>
              </div>
            ))}
          </div>
        ...
        {isHost && (
          <div className="bg-card/50 backdrop-blur-sm border-2 border-border rounded-xl p-6">
            <Button
              ...
              disabled={!canStart}
              className="w-full h-16 text-lg font-bold bg-transparent hover:bg-[#E50012]/10 text-white border-2 border-white rounded-xl disabled:opacity-30 disabled:cursor-not-allowed transition-all duration-200 hover:border-[#E50012] hover:text-[#E50012] disabled:hover:border-white disabled:hover:text-white"
            >
              <Play className="w-6 h-6 mr-2" />
              ゲームを開始する
            </Button>
            {!canStart && players.length >= 3 && (
              <p className="text-xs text-muted-foreground text-center mt-3">全員が準備完了するまでお待ちください</p>
            )}
          </div>
        )}

        {/* Share Info */}
        <div className="bg-card/50 backdrop-blur-sm border-2 border-border rounded-xl p-4">
          <div className="flex items-start gap-3">
            ...
            <Button
              ...
              className="bg-transparent hover:bg-white/5 border-2 border-white/70 text-white hover:border-white transition-all"
            >
              {copied ? (
                <>
        ...
        <div className="max-w-2xl mx-auto">
          {isHost ? (
            <Button
              onClick={handleStartGame}
              disabled={!canStart}
              className="w-full h-14 text-lg font-bold bg-transparent hover:bg-[#E50012]/10 border-2 border-white/80 rounded-xl disabled:opacity-30 disabled:cursor-not-allowed transition-all duration-200 hover:border-[#E50012] hover:text-[#E50012] disabled:hover:border-white/80 disabled:hover:text-white text-white"
            >
