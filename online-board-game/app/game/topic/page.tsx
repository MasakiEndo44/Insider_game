    ...
    return (
      <div className="min-h-screen circuit-bg circuit-pattern p-4 flex flex-col items-center justify-center">
        <div className="max-w-md w-full space-y-6 animate-fade-in">
          {/* Common Player Card */}
          <div className="bg-card/50 backdrop-blur-sm border-2 border-[#10B981] rounded-2xl p-8 space-y-6 text-center">
            <div className="flex justify-center">
              <div className="w-24 h-24 rounded-full bg-[#10B981]/20 flex items-center justify-center">
                <Image src="/images/common-icon.png" alt="庶民" width={60} height={60} />
              </div>
            </div>

            <h1 className="text-2xl font-black text-white">あなたは庶民です</h1>

            <div className="space-y-3 text-white">
              <p className="text-base leading-relaxed">質問フェーズでマスターに質問して、お題を当てましょう。</p>
              <p className="text-sm text-muted-foreground">お題は知らされていません。</p>
            </div>
          </div>
          ...
          <Button
            ...
            disabled={confirmed}
            className="w-full h-14 text-lg font-bold bg-transparent hover:bg-white/5 text-white border-2 border-white rounded-xl transition-all duration-200 hover:border-[#10B981] hover:text-[#10B981] disabled:opacity-50"
          >
  ...
  return (
    <div className="min-h-screen circuit-bg circuit-pattern p-4 flex flex-col items-center justify-center">
      <div className="max-w-md w-full space-y-6 animate-fade-in">
        {/* Topic Card */}
        <div
          className="bg-card/50 backdrop-blur-sm border-2 rounded-2xl p-8 space-y-6 text-center relative"
        ...
        >
          ...
          {isInsider && topicVisible && (
            <div className="absolute top-4 right-4">
              <div className="flex items-center gap-2 bg-[#E50012]/20 border border-[#E50012] rounded-full px-3 py-1">
                <Clock className="w-4 h-4 text-[#E50012]" />
                <span className="text-[#E50012] font-bold text-sm">{insiderTimer}</span>
              </div>
            </div>
          )}

          <h2 className="text-xl font-bold text-white">お題</h2>

          {/* Topic Display */}
          {topicVisible ? (
            <div className="bg-background/50 backdrop-blur-sm border-2 border-border rounded-xl p-6">
              <p className="text-4xl font-black text-white mb-2">{topic}</p>
              <p className="text-sm text-white/80">難易度: {difficulty}</p>
            </div>
          ) : (
            <div className="bg-background/50 backdrop-blur-sm border-2 border-border rounded-xl p-6">
              <EyeOff className="w-12 h-12 mx-auto text-white/60 mb-2" />
              <p className="text-sm text-white/80">お題は非表示になりました</p>
            </div>
          )}
          ...
          <div className="space-y-2 text-white text-sm leading-relaxed">
            {isMaster ? (
              <>
                ...
                <p className="text-xs text-muted-foreground">お題は常に表示されています</p>
              </>
            ) : (
              <>
                {topicVisible ? (
                  <p className="text-[#E50012]">10秒後に自動的にお題が消えます。覚えておいてください。</p>
                ) : (
        ...
        {/* Progress */}
        <div className="bg-card/30 backdrop-blur-sm border-2 border-border rounded-lg p-4 text-center">
